...
- **Calendar export** — the 📅 button in the Planner exports your next 30 days (scheduled decks, quick cards, and recurring cards with proper repeat rules) as an `.ics` file for Google Calendar, Apple Calendar, or Outlook, plus one-click "Add to Google Calendar" links per item. Items with a start time export as timed events; the rest are all-day.
- **Time-of-day scheduling** — give scheduled decks and quick cards an optional start time; the day panel sorts by time and calendar exports honor it.
- **Drag to reschedule** — grab the ⠿ handle on a scheduled deck and drop it on any calendar day to move it (its start time moves with it). Works with both touch and mouse.
...
```
index.html     — the app (React 18 + in-browser Babel)
manifest.json  — PWA manifest
sw.js          — service worker (offline cache)
icon.svg       — app icon
brain/         — optional assistant worker (Cloudflare): nightly check-ins,
                 Telegram chat, Google Calendar read/write
```
